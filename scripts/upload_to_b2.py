...
def run(files, prefix, manifest_prefix, skip_existing, dry_run, bucket=None, bucket_name=""):
    if dry_run:
        for f in files:
            print(f"  (dry-run) subiria -> {prefix}{f.name}")
        return 0, 0, 0
    ...
    for i, path in enumerate(files, start=1):
        ...
        if skip_existing:
            ...
            if existing is not None:
                ...
                failed += 1
                print(
                    f"  FAIL ya existe en B2 pero NO coincide "
    ...
    print()
    print(f"Resumen: ok={ok} skipped={skipped} failed={failed} de {len(files)} archivos")
...
def main() -> int:
    ...
    parser.add_argument("--manifest-prefix", type=str, default="manifests/uploads/")
    parser.add_argument("--no-skip-existing", action="store_true", help="Re-sube aunque ya exista en B2")
    parser.add_argument("--dry-run", action="store_true", help="Solo muestra que se subiria, sin tocar B2")
